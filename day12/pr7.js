...
// 1. 프로토타입 : 만들고자 하는 화면 스케치 = 피그마
    // P.60 지문
// 2. 메모리 설계 : 날짜 , 항목 , 금액 | 배열 = 표 / 객체 = 행 / 속성 = 열
let 가계부목록 = [ 
...
// 3. 기능설계 : 등록 , 출력
    ...
    // 2. 출력함수 : 페이지가 열리면/등록성공하면 , 현재 가계부목록(배열)에 저장된 객체들을 테이블에 출력
...
function 등록함수( ){
    ...
    // 2. 객체 구성
    let 객체 = { 날짜 , 항목 , 금액 } // vs 객체생성시 속성명과 속성값의 변수명이 같으면 생략 가능
...
function 출력함수( ){ 
    // 0. (백엔드 통신 코드) 추후에 백엔드로부터 배열 정보 가져온다.
    // 1. 어디에
    ...
    let html = ""
        for( let index = 0 ; index <= 가계부목록.length -1 ; index ++){
            ...
            // 주의할점 : html = 대입(기존자료사라짐) vs html += 누적(기존자료뒤에연결)
            html += `<tr>
                        <td> ${ 가계부.날짜 } </td>
                        <td> ${ 가계부.항목 } </td>
                        <td> ${ 가계부.금액 } </td>
