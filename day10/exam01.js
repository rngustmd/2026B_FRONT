/*
    함수(Function) : 미리 정의된 수/코드 , 상자안에 들어오는 수/코드 , 함(상자) 수(숫자/코드)
        ...
        * 매개변수(Parameter) : 함수호출하는 인자와 함수 실행하는 변수 연결 
        * 변수 : 하나의 값/자료 저장 , * 인자값 : 함수에 전달하는 값
        - 종류 : 1. 미리 만들어진 함수(라이브러리) 2. 내가 만든 함수
*/
// [1] 사용법
// 2. 미리 만들어진 함수 : 라이브러리 , 각 프로그램회사에서 제공
console.log( " 함수1 " )
// document.querySelector('body').innerHTML = ' 함수2 '
// 2. 내가 만드는 함수 : 정의함수 , 개발자(내가) 함수 직접 정의 , * 함수도 자료의 타입 이다.

// * 함수정의
// 1) function      2) 함수명 : 아무거나    3) ( 매개변수1 , 매개변수2 )    4) {    실행코드;   }
...
let 컵1 = 믹서기함수( '사과' ) // 믹서기함수에 사과를 인자값으로 전달하여 ??(무엇) 반환
let 컵2 = 믹서기함수( '포도' ) // 믹서기함수에 포도 인자값으로 전달하여 ???(무엇) 반환
...
// 4. 매개변수x , 반환값o       예] 추후에 다양한 함수들이 사용되는데 내가 만든 함수가 아니므로 무엇을 반환하는지 공문서/AI/구글 확인
...
if( true ){
    ...
    console.log( 전역변수 ); // { } 밖으로 나가는 건 불가능하지만 { } 안으로 들어가는 건 가능
    for( let i = 1 ; i <= 1 ; i ++ ){
...
// console.log( 지역변수1 ) // 오류o : 지역변수1은 if{ } 선언 했으므로 그 안에서만 사용 가능

// 2. 지역변수2 : if , for 과 다르게 function 은 return 반환 키워드
function func6( ){
    let 지역변수4 = '수원시';
    return 지역변수4;
}
